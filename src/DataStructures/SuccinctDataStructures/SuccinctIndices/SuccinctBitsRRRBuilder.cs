...
{
    /// <summary>
    /// Builds the bits sequence <see cref="SuccinctBitsRRR"/>
    ...
    public sealed class SuccinctBitsRRRBuilder
    {
        ...
        public SuccinctBitsRRRBuilder()
        {
            _bits = new BitsBuilder(0);
        }

        public SuccinctBitsRRRBuilder(BitsBuilder bits)
        {
            _bits = bits;
        }

        public SuccinctBitsRRRBuilder(nuint size)
        {
            _bits = new BitsBuilder(size);
        }

        public SuccinctBitsRRRBuilder(IReadOnlyList<byte> bytes)
        {
            _bits = new BitsBuilder(bytes);
        }

        public SuccinctBitsRRRBuilder Clear()
        {
            _bits.Clear();
            return this;
        }
        public SuccinctBitsRRRBuilder PushZeroes(int bitsCount)
        {
            _bits.AddUnsetBits(bitsCount);
            return this;
        }

        public SuccinctBitsRRRBuilder PushOnes(int bitsCount)
        {
            ...
            in QuasiSuccinctBits offsetPositionSamples)
        {
            for (nuint i = 0; i < blocksCount; i++)
            {
                ...
                var class2 =
                    SuccinctBitsRRR.ClassOfBlock(i, classValues);

                var offset2 =
                    SuccinctBitsRRR.OffsetOfBlock(
                        i,
                        @class,
                ...
                var block2 = SuccinctBitsRRR.FetchBlock(
                        i,
                        @class,
                        offsetPositionSamples,
                        offsetValues,
